...
实际发现 jewel 版本在 ceph-deploy install 安装 ceph 的时候已经安装了 ceph-radosgw。

```bash
# ceph-deploy 节点
$ cd /ceph-cluster
$ ceph-deploy install --rgw <gateway-node1> [<gateway-node2> ...] # 安装软件包
...
```bash
# rgw 节点
$ systemctl restart ceph-radosgw@rgw.centos192.service　# 重启 rgw 服务
...
```bash
# 尝试上传镜像（docker daemon 设置 --insecure-registry 192.168.111.0/24 并重启）
